...
export const AllTabsTableContent = ({ tabs }: { tabs: Tabs }) => {
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      const filteredItems = tabsList.filter((item: any) => {
        const modifiedName = "s" + item.name.toLowerCase();
        return (
          modifiedName.includes(lowerCaseSearch) ||
          item.longName.toLowerCase().includes(lowerCaseSearch)
        );
      });
      setCurrentItems(filteredItems);
    } else {
      setCurrentItems(
        tabsList && tabsList.slice(indexOfFirstItem, indexOfLastItem)
      );
    }
  }, [currentPage, tabsList, search]);
  ...
  return (
    <>
      <div className="flex justify-end">
        <div className="absolute rounded-md shadow-sm max-w-sm mt-[-4%]">
          <input
      ...
      <div className="flex justify-center border-2 rounded-2xl mt-4 overflow-x-auto">
