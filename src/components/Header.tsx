...
export default function Header() {
  ...
  const userVaultList: bigint[] | null | undefined =
    getAllVaultIDByOwner(address);
  ...
  const checkForLowReserveRatio = async () => {
    if (!userVaultList) {
      setHasError(false);
      return;
    }
    for (const item of userVaultList) {
      const data: any = await publicClient.readContract({
        ...VAULT_MANAGER_CONFIG,
        functionName: "getVaultDetails",
        args: [address, Number(item)],
      });

      const ratio = calculateVaultReserveRatio(
        data ? Number(data[5]) : 0,
        data ? Number(data[2]) : 0
      );
      // console.log(ratio);
      if (Number(ratio) !== 0 && Number(ratio) < 180) {
        setHasError(true);
        return;
      }
    }
  ...
  const calculateVaultReserveRatio = (
    reserveValue: number,
    osTab: number
  ): string => {
    const reserVal = reserveValue / 10 ** 18;
    const osVal = osTab / 10 ** 18;
  ...
  const handelCheckBoxClick = () => {
    ...
    localStorage.setItem("isChecked", address!);
  ...
  //useEffect(() => {
  //if (isDisconnected) {
  //toast.error("Connecting to a wallet has been cancelled.");
  //}
  //}, [isDisconnected]);

  useEffect(() => {
    const storedIsChecked = localStorage.getItem("isChecked");
    if (address && !userVaultList) {
      if (
        isConnected &&
        address &&
        (!storedIsChecked || storedIsChecked != address)
      ) {
        setOpenModal(true);
      }
    }
  ...
  useEffect(() => {
    const storedIsChecked = localStorage.getItem("isChecked");
    if (isConnected && !storedIsChecked) {
      // setOpenModal(false);
      setOpenCheckModal(true);
    }
  ...
  return (
    <>
                ...
                <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                  ...
                  <div className="hidden sm:ml-6 sm:block">
                    <div className="flex space-x-4">
                      {navigation.map((item) => (
                        <>
                          {item.name === "Your Vaults" && hasError ? (
                            <div className="flex flex-row items-center bg-red-100 px-2 py-1/2 ring-1 rounded-xl ring-red-500">
                              <img width={10} src="/icons/Error.png" />
                              <Link
                                key={item.name}
                                href={item.href}
                            ...
                            <div className="flex flex-row items-center  px-2 py-1/2  rounded-xl">
                              <Link
                                key={item.name}
                                href={item.href}
                                className={classNames(
      ...
      {/* <Modal show={openModal} onClose={() => setOpenModal(false)}>
        <Modal.Body className="pt-">
          <div className="flex justify-center">
            <img className="h-auto max-w-[150px]" src="/icons/sign.png" />
          </div>
          <p className="text-3xl text-center space-y-2 mt-10">Welcome</p>
          <div className="space-y-2 px-5 mt-5">
            <p className="text-center">
              It looks like you are new to ShiftCTRL or are using a new device
              to connect.
            </p>
            <p className="text-center">
              For added security, please sign a message to verify your ownership
              of the wallet to continue.
            </p>
          </div>
          <div className="flex flex-col items-center justify-center border-0 mt-5">
            <button
              className="bg-black text-white py-3 px-5 rounded-3xl w-1/4 my-2"
              onClick={() => signMessage({ message })}
            >
              Sign Message
            </button>
            <button className="underline" onClick={() => handelDisconnect()}>
              Disconnect
            </button>
          </div>
        </Modal.Body>
      ...
      {/* Checkbox modal */}
      <Modal show={openCheckModal} onClose={() => setOpenCheckModal(false)}>
        <Modal.Body className="pt-">
          <div className="flex justify-center">
            <img className="h-auto max-w-[150px]" src="/icons/Warning.png" />
          </div>
          <p className="text-3xl text-center space-y-2 mt-10">Warning!</p>
          <div className="space-y-2 px-5 mt-5">
            <p className="text-center">
              Operating vaults involves risk such as cyber security risk,
              volatility risk, etc, that may lead to financial losses,{" "}
              <span className="font-bold">Please use at your own risk</span>
            </p>
            <div className="flex justify-center mx-auto my-3">
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="remember">I understand the risk</label>
              </div>
            </div>
          </div>
          <div className="flex flex-col items-center justify-center border-0 mt-5">
