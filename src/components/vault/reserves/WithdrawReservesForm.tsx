import { VAULT_MANAGER_CONTRACT } from "@/app/helpers";
import abi from "@/app/helpers/ProtocolVault.json";
import { ethers } from "ethers";
import { useEthersSigner } from "@/utils/useEthersSigner";
import { useAccount } from "wagmi";
import { useState } from "react";

export const WithdrawReservesForm = () => {
  const { address, isConnected } = useAccount();
  const signer = useEthersSigner();
  const [vaultId, setVaultId] = useState<number>();
  const [collAmount, setColAmount] = useState<number>();

  const depositReserves = async () => {
    if (!address || !isConnected) {
      alert("Please connect your wallet");
      return;
    }
    const ContractInstance = new ethers.Contract(
      VAULT_MANAGER_CONTRACT,
      abi,
      signer
    );

    const tx = await ContractInstance.adjustColl(vaultId, collAmount, true, {
      gasLimit: 800000,
    });

    console.log(tx);
  };
  return (
    <div>
      <label className="text-sm font-medium">Withdraw amount</label>
      <div className="relative mt-2 rounded-md shadow-sm">
        <input
          type="text"
          name="price"
          id="price"
          className="block w-full rounded-xl border-0 py-1.5 pl-4 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6"
          placeholder="Enter a deposit amount"
        />
        <div className="absolute px-4 inset-y-0 right-0 flex items-center">
          wBTC
        </div>
      </div>
      <p className="text-sm my-4">Available: 2.123456789012345678</p>
      <button className="mt-10 w-full bg-black text-white py-2 px-5 rounded-3xl">
        Withdraw reserves
      </button>
    </div>
  );
};
