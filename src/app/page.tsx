...
const AllTabsTableContent = (tabs: any) => {
  ...
  const tabsList = Object.values(AllTabs);
  ...
  useEffect(() => {
    ...
    setTotalPages(Math.ceil(tabsList.length / itemsPerPage));
    ...
    <>
      <div className="flex justify-center border-2 rounded-2xl mt-4">
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 ">
          ...
          <tbody>
            {currentItems.map(
              (item: { byte: string; name: string; longName: string }) => (
                <TabsTableData
                  tabByte={item.byte}
                  tabName={item.name}
                  tabLongName={item.longName}
                />
              )
            )}
          </tbody>
  ...
  return (
    <>
      <tr className="border-t-2">
        ...
        <td className="px-6 py-4">{oraclePrice && oraclePrice.toString()}</td>
        <td className="px-6 py-4">
